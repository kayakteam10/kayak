...
class CarRepository {
    ...
    `;
        ...
        query += ` ORDER BY c.price_per_day ASC LIMIT 50`;
    ...
    async reserveCar(carId, pickupDate, returnDate) {
        ...
        try {
            ...
            const [carRows] = await connection.execute(
                `SELECT * FROM cars WHERE id = ? AND is_available = TRUE FOR UPDATE`,
                [carId]
            );
            ...
            await connection.execute(
                `UPDATE cars SET is_available = FALSE WHERE id = ?`,
                [carId]
            );
    ...
    async releaseCar(carId) {
        const query = `UPDATE cars SET is_available = TRUE WHERE id = ?`;
    ...
    async getCarTypes(location) {
      ...
      FROM cars
      WHERE location LIKE ? AND is_available = TRUE
      GROUP BY car_type
    `;
     ...
     * @returns {Promise<Object>}
     */
    async updateAvailability(carId, isAvailable) {
        const query = `UPDATE cars SET is_available = ? WHERE id = ?`;
        const [result] = await this.db.execute(query, [isAvailable, carId]);
